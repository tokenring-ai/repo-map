  ...
  if (files.size > 0) {
   ...
   for (const file of files) {
    try {
      ...
      if (code === null || code === undefined) { // Check if file content is valid
        console.error(`[RepoMapService] Error: Could not read content for file ${file}. Skipping.`);
        continue;
      }
      ...
      console.error(`[RepoMapService] Error processing file ${file}:`, error);
 ...
 extractSymbols(tree, lang) {
  ...
  function traverse(depth = 0, parentSymbol = null) {
   ...
   if (kind && kind !== "program") {
    ...
    if (node.type === "method_definition" && parentSymbol && parentSymbol.kind === "class") {
