...
export async function execute({
 ...
 const symbolDescription = parentClass ? `${symbolType} '${symbolName}' in class '${parentClass}'` : `${symbolType} '${symbolName}'`;
 chatService.infoLine(`[RepoMap] Modifying ${symbolDescription} in ${filePath}`);
 ...
 try {
  ...
  if (parentClass) {
   ...
   const existingSymbol = findSymbolInClass(classSymbol.node, symbolName, symbolType);

   if (existingSymbol) {
    newCode = replaceSymbol(originalCode, existingSymbol, content);
   } else {
    newCode = createSymbolInClass(originalCode, classSymbol, content, symbolType);
   }
  ...
  if (success) {
   ...
   chatService.infoLine(`[RepoMap] Successfully modified ${symbolDescription} in ${filePath}`);
...
function findSymbol(tree, symbolName, symbolType) {
 const symbolTypes = {
  ...
  variable: ["variable_declarator", "lexical_declaration", "variable_declaration"],
 ...
 function traverse() {
  ...
  if (targetTypes.includes(node.type)) {
   ...
   if (node.type === "export_statement") {
    ...
    if (symbolType === 'constructor' && nameNode && nameNode.text === 'constructor') {
 ...
 function traverse() {
  ...
  if (cursor.gotoFirstChild()) {
   do {
    ...
    if (cursor.currentNode.type !== "class_declaration" && cursor.currentNode.type !== "class_definition") {
     const result = traverse();
     if (result) return result;
    }
   } while (cursor.gotoNextSibling());
...
function replaceSymbol(originalCode, existingSymbol, newContent) {
 ...
 if (startLine === endLine) {
  ...
  lines[startLine] = line.substring(0, startColumn) + newContent + line.substring(endColumn);
 } else {
  ...
  newContentLines[newContentLines.length - 1] = newContentLines[newContentLines.length - 1] + lastLine;
...
function createSymbolInClass(originalCode, classSymbol, content, symbolType) {
 ...
 const classLines = lines.slice(classSymbol.startPosition.row, classEndLine + 1);
 ...
 lines[insertLine] = beforeClosingBrace + '\n' + indentedContent + '\n' + classIndent + afterClosingBrace;
...
export const description = "Create or replace specific symbols (functions, classes, variables, methods, constructors, properties) in source code files using Tree-sitter parsing. Supports nested symbols like methods within classes.";
